...
\begin{df}\label{def:smooth_vector_bundle}
...
\begin{df}
	A \textbf{subriemannian structure over a manifold} $ M $ is a pair $ (\mathcal{H}, <\cdot,\cdot>)$, where $ \mathcal{H}\subset TM $ is a distribution and $ <\cdot,\cdot> $ is a section of the bundle $ T^2_0 \mathcal{H} \xrightarrow[]{\pi} M, $ which restricts to a positive definite symmetric bilinear form in each fiber (see \cite{hatcher2003vector}, Ch. 1, page 12).
...
\begin{proof}
	...
	\end{dmath*}
	and evaluating this expression in $ \Phi_1(t)\circ\Phi_2(t) $ gives the desired result.
\end{proof}
...
\begin{align*}
 \operatorname{Box}_p(\epsilon):= \left\{ \Phi_k(t_k)\circ\cdots\circ \Phi_2(t_2)\circ\Phi_1(t_1)(p) \ | \ |t_i|<\epsilon, \ i=1,\dots,k \right\}\\ \subset \operatorname{B}(p, k\epsilon):= \left\{ q\in M \ | \ d(p,q)<k\epsilon \right\}.  
\end{align*}
