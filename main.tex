...
\begin{align}
 [hX, hY] = h([X,Y])\oplus -\eta(\Omega(X,Y)), \label{eq:horizontal-horizontal}\\
 [\sigma\xi, \sigma\chi] = -\sigma([\xi,\chi]), \label{eq:vertical-vertical} \\
 [hX,\sigma \xi] = \eta(D_X\xi). \label{eq:horizontal-vertical} 
\end{align}
...
\begin{thm}
	Suppose that $ \pi: Q \rightarrow M $  is a principal $ G $-bundle and $ \rho: G \rightarrow \operatorname{GL} (E) $ is a representation of $ G $ on a vector space $ E. $ Let $ c: I \rightarrow M $ be a curve on $ M. $ A curve $ \psi: I \rightarrow \rho(Q) $ lying over $ c $ is covariantly constant (see Definition \ref{df:covariantly_constant}) if and only if $ \psi(t)= [q(t), \psi_0] $, where $ \pi\circ q = c,  $ and $ \psi_0\in E $ is constant.
\end{thm}
\begin{proof}
...
\begin{cor}[First Wong's equation]
	$\alpha: I \rightarrow T^*Q$ is a normal sub-Riemannian geodesic if and only if $ \alpha $ is covariantly constant, that is, $$ D_{\dot{c}}\lambda =0, $$ where $ c=\pi\circ\pi_{T^*Q}\circ\alpha, $ and $ \lambda(t)=[ \pi_{T^*Q}\circ\alpha(t), J\circ\alpha(t)]. $ 
\end{cor}
...
\begin{cor}
	[Second Wong's equation] $ \alpha: I \rightarrow T^*Q $ is a normal sub-Riemannian geodesic if and only if it satisfies the equation
