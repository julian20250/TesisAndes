...
\end{prop}
\begin{proof}
	Assign to each $ F\in C_G(Q,E) $ the section $s $ defined by $s(m)=[q, F(q)]\in \rho(Q)_m $ whenever $ \pi(q)=m\in M. $  This is well-defined because if $ h= q\cdot g, $ $ [h,F(h)]=[q\cdot g, \rho(g^{-1})F(q)]=[q,F(q)]$. On the other hand, if $s: M \rightarrow \rho(Q),$ $s(m)=[s_1(m), s_2(m)]$ is a section, we assign to it the corresponding $G$-equivariant function as $F(q):= \rho(g^{-1})s_2(m),$ where $ \pi(q)=m $ and $ q=s_1(m)\cdot g $. This map is $ G $-equivariant because $$F((s_1(m)\cdot g_1)\cdot g_2) = \rho(g_2^{-1} g_1^{-1})s_2(m) = \rho(g_2^{-1})F(s_1(m)\cdot g_1).$$ We can check that these assignations are one-to-one because the corresponding section of $F(q)= \rho(g^{-1})s_2(m)$ is $s(m)=[q,\rho(g^{-1}) s_2(m)]=[q\cdot g^{-1}, s_2(m)]=[s_1(m), s_2(m)],$ while the corresponding $ G $-equivariant map of $ s(m)=[q, F(q)] $  is $ \rho(e) F(q)=F(q). $ 
\end{proof}
...
\begin{prop}\label{prop:isomorphism_vb}
	The map $ \sigma: \operatorname{Ad} (Q) = (Q\times \mathfrak{g})/G \rightarrow V/G $ is an isomorphism between bundles over $ M. $ 
\end{prop}
\begin{proof}
