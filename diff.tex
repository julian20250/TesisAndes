...
%DIF DEL previous.tex   Mon Jul 11 14:55:37 2022
%DIF ADD main.tex       Mon Jul 18 18:28:36 2022
% Title of Document: 
% Start Date: 
%%%%%%%%%%%%%%%%%%%%%%%%
%%%FINAL CHECK%%%%%%%%%
...
%DIF 43c43
%DIF < \setlength{\parskip}{1em}
%DIF -------
%\setlength{\parskip}{1em} %DIF > 
%DIF -------
% Skip indentation
%DIF 45c45
...
\newpage
\begin{abstract}
		The goal of this work is to \DIFdelbegin \DIFdel{introduce the main theorems }\DIFdelend \DIFaddbegin \DIFadd{explain the elements }\DIFaddend of sub-Riemannian geometry \DIFdelbegin \DIFdel{, and for that we will cover first the topic of distributions, stating the Frobenius ' theoremand introducing the idea of foliation. Later on, }\DIFdelend \DIFaddbegin \DIFadd{and present some of its applications. We start with the concept of distribution, then explain the Frobenius theorem. After that }\DIFaddend we focus on \DIFdelbegin \DIFdel{the }\DIFdelend sub-Riemannian structures, and prove \DIFdelbegin \DIFdel{Chow's }\DIFdelend \DIFaddbegin \DIFadd{the Chow }\DIFaddend theorem, central in the study of connectivity through horizontal geodesics. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{In the }\DIFaddend last part of \DIFdelbegin \DIFdel{this work focuses on }\DIFdelend \DIFaddbegin \DIFadd{the thesis we consider the }\DIFaddend $ G $-principal bundles endowed with compatible sub-Riemannian structures\DIFdelbegin \DIFdel{. We use this construction with ideas of symplectic geometry to prove the Theorem }\DIFdelend \DIFaddbegin \DIFadd{, prove the theorem }\DIFaddend on normal geodesics of bundle type sub-Riemannian metrics\DIFdelbegin \DIFdel{and to }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend study Wong's equations. \DIFdelbegin \DIFdel{This review of sub-Riemannian topics }\DIFdelend \DIFaddbegin \DIFadd{Our exposition }\DIFaddend is based mostly \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend \cite{montgomery2002tour}.
\end{abstract}
\newpage
...
Riemannian geometry is an \DIFdelbegin \DIFdel{intense field of study }\DIFdelend \DIFaddbegin \DIFadd{intensively studied field }\DIFaddend due to the fact that it \DIFdelbegin \DIFdel{is the }\DIFdelend \DIFaddbegin \DIFadd{provides a }\DIFaddend generalization of the idea of \DIFdelbegin \DIFdel{metric and distance onto general geometric spaces}\DIFdelend \DIFaddbegin \DIFadd{distance from euclidean space to manifolds}\DIFaddend . 
This characteristic makes Riemannian geometry one of the preferred formalisms to work in physics \DIFdelbegin \DIFdel{' }\DIFdelend topics like general relativity and quantum mechanics. 
In \DIFdelbegin \DIFdel{this field }\DIFdelend \DIFaddbegin \DIFadd{Riemannian geometry }\DIFaddend we start with a manifold \DIFdelbegin \DIFdel{, }\DIFdelend endowed with a \DIFdelbegin \DIFdel{metric, which is a section of the $ (0,2) $}\DIFdelend \DIFaddbegin \emph{\DIFadd{a metric tensor field}} \DIFadd{$g$, where $g$ is a $(0,2)$}\DIFaddend -tensor \DIFdelbegin \DIFdel{bundle which }\DIFdelend \DIFaddbegin \DIFadd{field that }\DIFaddend is symmetric and \DIFdelbegin \DIFdel{non-degenerate }\DIFdelend \DIFaddbegin \DIFadd{positive definite }\DIFaddend at all points of the manifold.  \DIFdelbegin \DIFdel{The non-degeneracy condition can be weakened, and this variation of the definition of Riemannian metric arises }\DIFdelend \DIFaddbegin \DIFadd{If we replace the condition ``positive definite'' with the condition ``non-degenerate'' we arrive at }\DIFaddend the field of study known as pseudo-Riemannian \DIFdelbegin \DIFdel{metric. In this case some }\DIFdelend \DIFaddbegin \DIFadd{geometry. Some }\DIFaddend important examples raise up \DIFaddbegin \DIFadd{in this alternative theory}\DIFaddend , like the \DIFdelbegin \DIFdel{Lorentz/Minkowski geometry , the most widely used metric }\DIFdelend \DIFaddbegin \DIFadd{geometry of Minkowski space or, more generally, Lorenzian geometry; both being widely used }\DIFaddend in special relativity. 
An extensive mathematical development of pseudo-Riemannian geometry can be found \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend \cite{o1983semi}.

Another variation of Riemannian metric arises when the condition of being a section of the $(0,2)$-tensor bundle is weakened to consider instead a continuous map $F: TM \rightarrow \mathbb{R}$, smooth on the set of nonzero vectors, whose restriction to each tangent space $T_pM$ is a norm \cite[p.~47]{lee2018introduction}. This definition is known as Finsler metric \DIFdelbegin \DIFdel{, and an approach to the consequences of this definition can be found on \mbox{%DIFAUXCMD
\cite{bao2012introduction}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(see e.\, g. \mbox{%DIFAUXCMD
\cite{bao2012introduction}}\hspace{0pt}%DIFAUXCMD
)}\DIFaddend .

The variation of  \DIFdelbegin \DIFdel{Riemannian geometry }\DIFdelend \DIFaddbegin \DIFadd{the Riemannian geometry theory }\DIFaddend that will be covered throughout this text arises when we consider a sub-bundle (called distribution) of the tangent bundle of a given manifold: in this case, the metric \DIFaddbegin \DIFadd{tensor field }\DIFaddend will be a section of the $ (0,2) $-tensor bundle of this sub-bundle, with the same properties \DIFdelbegin \DIFdel{than in the case of }\DIFdelend \DIFaddbegin \DIFadd{as the ones used in }\DIFaddend Riemannian geometry (non-degeneracy and symmetry). This formulation receives the name of sub-Riemannian geometry, and it will be the main focus of this text. Before introducing the concept of sub-Riemannian structure, \DIFdelbegin \DIFdel{we focus }\DIFdelend in Section \ref{sec:1} \DIFdelbegin \DIFdel{on explaining the idea of distribution , their different definitions and associated results. To do this, we define a distribution }\DIFdelend \DIFaddbegin \DIFadd{we introduce the concept of distribution on manifold, describe distributions }\DIFaddend in terms of local frames and 1-forms, \DIFdelbegin \DIFdel{we }\DIFdelend explain when a distribution is involutive, integrable and completely integrable, and use the Frobenius \DIFdelbegin \DIFdel{'s }\DIFdelend theorem to connect these three concepts, while discussing when the points of a given manifold are connected via horizontal curves, \textit{i.e.}, curves whose tangent vector is always contained in the distribution.

\DIFdelbegin \DIFdel{Later on, in }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend Section \ref{sec:sub-Riemannian_geometry} we introduce the concept of sub-Riemannian structure, and define the horizontal distance as the infimum of the length of all horizontal curves that connect two given points in the manifold, in analogy with Riemannian distance. We \DIFdelbegin \DIFdel{then introduce }\DIFdelend \DIFaddbegin \DIFadd{define }\DIFaddend the bracket-generating distributions \DIFdelbegin \DIFdel{as a special class of distribution such that there is a local frame that locally spans the tangent bundle through successive applications of Lie brackets, and }\DIFdelend \DIFaddbegin \DIFadd{characterized by the property that taking the span of the vector fields obtained by the successive application of the Lie bracket to the vector fields tangent to the distribution we get the tangent bundle, then }\DIFaddend we state and \DIFdelbegin \DIFdel{show the Chow 's }\DIFdelend \DIFaddbegin \DIFadd{prove the Chow }\DIFaddend theorem, which states that a sufficient condition for all points to be connected through horizontal curves is that the distribution is bracket-generating. Therefore, the distance between two points in a sub-Riemannian structure is always well-defined when the \DIFdelbegin \DIFdel{subjacent }\DIFdelend \DIFaddbegin \DIFadd{corresponding }\DIFaddend distribution is bracket-generating. 

\DIFdelbegin \DIFdel{In order to work with principal $ G $-bundles endowed with metric of bundle type, in Section \ref{sub:metrics_on_principal_bundles} we start introducing what is }\DIFdelend \DIFaddbegin \DIFadd{Section \ref{sub:metrics_on_principal_bundles} is devoted to the study of sub-Riemannian metrics on bundles.
We start with the definition of }\DIFaddend a submersion, and \DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{explain how to }\DIFaddend endow it with a sub-Riemannian structure in the total space (equivalently, a Riemannian structure in the base space). With these tools, we \DIFdelbegin \DIFdel{start the topic of }\DIFdelend \DIFaddbegin \DIFadd{study }\DIFaddend metrics on principal bundles \DIFdelbegin \DIFdel{explaining aspects of Lie groups and their actions, and we }\DIFdelend \DIFaddbegin \DIFadd{in Subsection~\ref{sub:metrics_on_principal_bundles}, and }\DIFaddend give some examples of principal $ G $-bundles in Section \ref{sub:examples_of_principal_g_bundles}. 

Following the steps of Riemannian geometry, in Section \ref{sub:theorem_on_normal_geodesics} we proceed studying the normal geodesics associated to sub-Riemannian structures, and in order to do that we make a brief introduction to symplectic geometry. This will allow to use the Hamiltonian formalism to characterize the horizontal geodesics as integral curves of the vector field associated to the sub-Riemannian Hamiltonian, which is defined in the same way than the Riemannian Hamiltonian, using the sub-Riemannian metric instead. With this formalism we proceed proving \DIFdelbegin \DIFdel{the Theorem }\DIFdelend \DIFaddbegin \DIFadd{a theorem }\DIFaddend on normal geodesics of bundle type sub-Riemannian metrics, which provides a way to identify normal (horizontal) geodesics in the (base) total space for a principal $ G $-bundle whose metric satisfies some special properties.

The cases where the conditions of the Theorem on normal geodesics of bundle type sub-Riemannian metrics do not apply are covered in Section \ref{sec:classical_particles_in_yang_mills_fields}, where we characterize the normal horizontal geodesics of a principal $ G $-bundle using the Wong \DIFdelbegin \DIFdel{'s equations. These are a family }\DIFdelend \DIFaddbegin \DIFadd{equations, a system }\DIFaddend of differential equations \DIFdelbegin \DIFdel{over }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the quotient of the \DIFdelbegin \DIFdel{cotangent space and the }\DIFdelend \DIFaddbegin \DIFadd{total space of the cotangent bundle with respect to the }\DIFaddend action of the Lie group $ G $ over it.
\DIFaddbegin 

\section*{\DIFadd{Acknowledgments}}
\DIFadd{I would like to express my deepest gratitude to professors Mikhail Malakhaltsev, Andres Vargas, Alexander Trejos and Jean Carlos Cortissoz for their helpful suggestions and comments during the writing process for this text.
}\DIFaddend \section{Distributions}
\DIFaddbegin 

\DIFaddend \label{sec:1}
...
\begin{df}(\cite{luke2013vector}, section 1.3)\label{def:smooth_vector_bundle}
	The triple $ (E, M, \pi: E \rightarrow {M}) $ is a \textit{smooth vector bundle} if $ E,M $ are smooth manifolds, $ \pi $ is a smooth surjection; the fibers $ \pi^{-1}(p) $ for $ p\in M $  have the structure of finite dimensional vector spaces; and for every point $ p\in M $, there is an open neighborhood $ U $ of $ p $, a natural number $ k $ \DIFaddbegin \DIFadd{(known as the rank of the vector bundle) }\DIFaddend and a diffeomorphism $ \phi: U\times \mathbb{R}^k \rightarrow  \pi^{-1}(U),$ such that for all $ q\in U $ the following conditions hold:  
...
equipped with manifold structure given by \DIFdelbegin \DIFdel{the one }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend of $ M. $ It is called a bundle because the triple $ (TM, M,\pi:TM \rightarrow {M}) $ satisfies the definition of smooth vector bundle, where $ \pi:TM \rightarrow {M} $  is the projection function, that assigns each tangent vector to the point where it is based. Each fiber of this bundle is a tangent space based on a point, and the tangent bundle is in particular a vector bundle\DIFaddbegin \DIFadd{, }\DIFaddend because each fiber has \DIFdelbegin \DIFdel{structure of }\DIFdelend \DIFaddbegin \DIFadd{the structure of a }\DIFaddend finite dimensional real vector space.



There is another important class of smooth vector bundles, namely, the tensor bundles, \DIFdelbegin \DIFdel{that are defined following \mbox{%DIFAUXCMD
\cite{wendl2008lecture}}\hspace{0pt}%DIFAUXCMD
, examples 2.4 and 2.5.
}%DIFDELCMD < \begin{df} 
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \DIFadd{which will be covered next.
}\begin{df}[\cite{wendl2008lecture}, Example 2.4] 
	\DIFaddend If $ (E,M,\pi:E \rightarrow {M}) $ is a smooth vector bundle, the \DIFdelbegin \DIFdel{dual bundle }\DIFdelend \DIFaddbegin \textit{\DIFadd{dual bundle}} \DIFaddend of this bundle is another smooth vector bundle $ (E^*,M,\pi^*:E^*\rightarrow {M}) $, \DIFdelbegin \DIFdel{known as }\textit{\DIFdel{dual bundle}}%DIFAUXCMD
...
\end{df}
\DIFdelbegin %DIFDELCMD < \begin{df}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \begin{df}[\cite{wendl2008lecture}, Example 2.5]
	\DIFaddend If $ (E,M,\pi:E \rightarrow {M}) $ is a smooth vector bundle, the \textit{$ (k,l)- $tensor bundle} associated to this bundle, with $ k,l\geq0 $, is defined as the smooth vector bundle $ (T^k_l E, M, \pi_{kl}: T^k_l E \rightarrow {M}) $, whose fibers are the vector spaces of $ (k,l) -$tensors of the fibers of $ E, $ that is,     
...
\end{df}
\DIFdelbegin \DIFdel{For an arbitrary smooth vector bundle $ (E,B,\pi:E \rightarrow {B}), $ a }\DIFdelend \DIFaddbegin \begin{df}
\DIFadd{A }\DIFaddend sub-bundle \DIFdelbegin \DIFdel{of this bundle $ (E',B',\pi':E' \rightarrow {B'}) $  }\DIFdelend \DIFaddbegin \DIFadd{$ (E',B',\pi':E' \rightarrow {B'}) $ of a vector bundle $  (E,B,\pi:E \rightarrow {B})  $   }\DIFaddend is a smooth vector bundle that satisfies $ E'\subset E $, $ B'\subset B $, $ \pi|_{E'}=\pi' $ and the \DIFaddbegin \DIFadd{pair of }\DIFaddend inclusion maps 
\begin{center}
...
%DIFDELCMD < %%%
\DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{define }\DIFaddend a morphism of vector bundles.
\DIFaddbegin \end{df}
\DIFaddend \begin{df}
	...
	The fiber of $ \mathcal{H} $ at a point $ p\in M $ is denoted as \DIFdelbegin \DIFdel{$ H_p. $ 
}\DIFdelend \DIFaddbegin \DIFadd{$ \mathcal{H}_p. $ 
}\DIFaddend \end{df}

The concept of distribution can be defined in an alternative but equivalent  \DIFaddbegin \DIFadd{fashion using 1-forms }\DIFaddend (see \cite{lee2003introduction}, Ch. 19, \DIFdelbegin \DIFdel{lemma }\DIFdelend \DIFaddbegin \DIFadd{Lemma }\DIFaddend 19.5) \DIFdelbegin \DIFdel{fashion using 1-forms, }\DIFdelend with the help of the following \DIFdelbegin \DIFdel{proposition}\DIFdelend \DIFaddbegin \DIFadd{Proposition}\DIFaddend .

\begin{prop}[Distribution (using 1-forms)]
	Suppose that $ M $ is a smooth \DIFdelbegin \DIFdel{$ m- $dimensional }\DIFdelend \DIFaddbegin \DIFadd{$ m $-dimensional }\DIFaddend manifold, and $ \mathcal{H} \subset TM$ is a distribution of rank $k$. Then $ \mathcal{H} $ is smooth if and only if for every point $ p\in M $ there is a neighborhood $ U $ of that point where \DIFaddbegin \DIFadd{there }\DIFaddend exist $n-k$ smooth 1-forms $ \omega^1,\dots,\omega^{n-k}, $ such that for all $ q\in U $,

	$$ \mathcal{H}_q = \operatorname{Ker} \omega^1|_q\cap \cdots \cap\operatorname{Ker} \omega^{n-k}|_q.  $$ 
\end{prop}

The \DIFdelbegin \DIFdel{following }\DIFdelend \DIFaddbegin \DIFadd{next }\DIFaddend proposition gives an alternative way to represent a smooth distribution in terms of local frames (\cite{lee2003introduction}, Ch. 19, page 491).

\begin{prop} [Distribution (using local frames)]
	$ \mathcal{H}\subset TM $ is a smooth distribution over $M  $ if and only if  for each point $ p\in M $, there is an open neighborhood $ U $ of $ p $  and smooth vector fields $ X_1,\dots,X_n: U \rightarrow {TM} $ that at each point  $ q\in U $ form a \DIFdelbegin \DIFdel{base }\DIFdelend \DIFaddbegin \DIFadd{basis }\DIFaddend for $ \mathcal{H}_q. $ 
\end{prop}
...
\end{ex}
\begin{rem}
	The last example is of particular interest because of its connection with the \DIFdelbegin \DIFdel{isoperimetrical }\DIFdelend \DIFaddbegin \DIFadd{isoperimetric }\DIFaddend problem. For more information see (\cite{montgomery2002tour}, Ch. 1).
\end{rem}
...
\subsection{Frobenius \DIFdelbegin \DIFdel{' }\DIFdelend Theorem}%
\label{sub:frobenius_theorem}


\DIFdelbegin \DIFdel{Now the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend natural question that arises \DIFdelbegin \DIFdel{is}\DIFdelend \DIFaddbegin \DIFadd{from the definition of horizontal curves is that}\DIFaddend , if given two points on a manifold with a given distribution, is there a smooth horizontal curve that joins them? 
...
There are distributions over a manifold that do not have integral manifolds, as \DIFdelbegin \DIFdel{is }\DIFdelend shown in the \DIFdelbegin \DIFdel{following example }\DIFdelend \DIFaddbegin \DIFadd{example below }\DIFaddend (\cite{lee2003introduction}, Ch. 19, example 19.1).
...
\begin{ex}[Distribution without integral manifolds] \label{ex:no_integral_manifolds}
	...
	Therefore, as the integral curve of $ X $ is a straight line contained in the plane that is parallel to the \DIFdelbegin \DIFdel{$ xz- $plane}\DIFdelend \DIFaddbegin \DIFadd{$ xz $-plane}\DIFaddend , there is a segment of this line that is contained in $ N $, and for every point in this segment \DIFdelbegin \DIFdel{, }\DIFdelend the integral curves of $ Y $ that passes through them are straight lines parallel to the \DIFdelbegin \DIFdel{$ y- $axis}\DIFdelend \DIFaddbegin \DIFadd{$ y $-axis}\DIFaddend , so there must be an open neighborhood of $ \textbf{x}  $ that contains a plane generated by the segments of the integral curves of $ X $ and $ Y $ that passes through $ \textbf{x}$ (see \DIFdelbegin \DIFdel{figure }\DIFdelend \DIFaddbegin \DIFadd{Figure }\DIFaddend \ref{fig:no_integral_manifolds}). However, the tangent plane at any point $ p $  of this plane  \DIFdelbegin \DIFdel{off }\DIFdelend of the segment induced by the integral curve of $ X $ is not equal to $ \mathcal{H}_p, $ since $ X $ depends \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend $ y $.   
\end{ex}
...
\begin{proof}
	...
	\end{dmath*}
	and for any $ q\in\varphi(U) $, $ t\mapsto (\theta_i)_t(q) $ is an integral curve of $ V_i, $ so the above expression is equal to $ V_i|_{\Phi(s^0)} f $, which shows that $ \partial/\partial s^i |_{s^0} $ and $ V_i $ are \DIFdelbegin \DIFdel{$ \Phi- $related}\DIFdelend \DIFaddbegin \DIFadd{$ \Phi $-related}\DIFaddend .   
	...
	Therefore, $ d\Phi_0 $ takes the basis $ \left( \partial/\partial s^1 |_0,\dots,\partial/\partial s^m |_0 \right) $ of $ T_0 \mathbb{R}^m $ to the basis $ ( V_1|_{\varphi(p)},  \dots,$ $ V_k|_{\varphi(p)}, \partial/\partial x^{k+1}|_{\varphi(p)}, \dots, \partial/\partial x^{m}|_{\varphi(p)}) $ of $ T_{\varphi(p)} \varphi(U). $ By the inverse function theorem, $ \Phi $ is a local diffeomorphism (in a neighborhood of $ 0 $), and $ \phi=\Phi^{-1} $ is a smooth coordinate chart that takes $ V_i $ to $ \partial/\partial s^i $, for $ i=1,\dots,k, $ and takes $ \partial / \partial x^{i} $ to $ \partial/\partial s^i $, for $ i=k+1,\dots,m. $ Thus, the smooth coordinate \DIFdelbegin \DIFdel{chart flat }\DIFdelend \DIFaddbegin \DIFadd{flat chart }\DIFaddend for $ \mathcal{H} $ in a neighborhood of $ p\in M $ is $ \phi\circ\varphi, $ and since this is independent of $ p $, for every point there is a smooth coordinate \DIFdelbegin \DIFdel{chart flat }\DIFdelend \DIFaddbegin \DIFadd{flat chart }\DIFaddend for $ \mathcal{H}, $ what makes $ \mathcal{H} $ a completely integrable distribution.

\end{proof}

The next proposition is one of the main consequences of Frobenius theorem, and is fundamental to study \DIFdelbegin \DIFdel{foliations, topic that }\DIFdelend \DIFaddbegin \DIFadd{the theory of foliations, which }\DIFaddend is going to be discussed \DIFdelbegin \DIFdel{below}\DIFdelend \DIFaddbegin \DIFadd{later on}\DIFaddend .
...
\begin{proof}(\cite{lee2003introduction}, Ch.\DIFaddbegin \DIFadd{~}\DIFaddend 19, \DIFdelbegin \DIFdel{proposition }\DIFdelend \DIFaddbegin \DIFadd{Proposition }\DIFaddend 19.16)
...
The curves that are considered in Definition \ref{df:horizontal_distance} are required to be absolutely continuous, which is a weaker condition than smoothness in the sense that the \DIFaddbegin \DIFadd{former kind of }\DIFaddend curve is differentiable in almost all points of its domain, not necessarily in all its domain. Nevertheless, the distance remains the same if we consider instead the set of all smooth curves (see \cite{montgomery2002tour}, page 23), but the motivation to consider the bigger set of absolutely continuous curves is due to the fact that the curves that realize the distance between two points are not always smooth.

\begin{df}
	Given a sub-Riemannian structure $ (\mathcal{H}, \langle\cdot,\cdot\rangle) $ over a manifold $ M $  (or Riemannian structure, if $ \mathcal{H}=TM $), it is said that an absolutely continuous horizontal curve $ \gamma:  [a,b] \rightarrow M$ \DIFdelbegin \DIFdel{, with $ \gamma(a)=p, \gamma(b)=q $ }\DIFdelend is a \textit{geodesic} if it \DIFdelbegin \DIFdel{realizes the distance between $ p $ and $ q $}\DIFdelend \DIFaddbegin \DIFadd{locally minimizes distances}\DIFaddend , that is, \DIFdelbegin \begin{displaymath} \DIFdel{\ell(\gamma)= d(p,q). }\end{displaymath}%DIFAUXCMD
%DIFDELCMD <  
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{there exists some $ \epsilon>0 $ such that $ d(\gamma(t_1),\gamma(t_2))=\ell(\gamma|_{t_1}^{t_2}) $, whenever $ |t_1-t_2|<\epsilon. $  
}\DIFaddend \end{df}
...
\DIFdelbegin %DIFDELCMD < \begin{df}[\cite{montgomery2002tour}, definition 2.4.1] 
%DIFDELCMD < 	%%%
...
where the error $ o(t_i^{w_i}) $ corresponds to the composition of the previous $ \Psi_1(t), \dots,\Psi_{i-1}(t) $ flows evaluated in the big \DIFdelbegin \DIFdel{$ O- $errors }\DIFdelend \DIFaddbegin \DIFadd{$ O $-errors }\DIFaddend of the previous sequential arguments. It remains to understand and bound these error terms, and for that we introduce the new variables $ s_i=\sigma_i(t), $ so that $ F_i(t_1,\dots,t_m)= s_i + o(s_i). $  
...
\end{proof}
The remaining \DIFdelbegin \DIFdel{containment }\DIFdelend \DIFaddbegin \DIFadd{inclusion }\DIFaddend in the Ball-Box theorem will not be proved in this text, as the proof of Chow's theorem only requires the first \DIFdelbegin \DIFdel{containment}\DIFdelend \DIFaddbegin \DIFadd{inclusion}\DIFaddend . The proof of the remaining \DIFdelbegin \DIFdel{containment }\DIFdelend \DIFaddbegin \DIFadd{inclusion }\DIFaddend can be found in \cite{montgomery2002tour}, section 2.7.

With the Ball-Box theorem, Chow's theorem can be derived easily.

\DIFdelbegin %DIFDELCMD < \begin{thm}[Chow's Theorem]
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \begin{thm}[Chow Theorem]
	\DIFaddend If $ \mathcal{H} $ is a bracket-generating distribution on a connected manifold $ M $, then any two points of $ M $ can be connected by a horizontal path.
\end{thm}
\begin{proof} (\cite{montgomery2002tour}, \DIFdelbegin \DIFdel{page }\DIFdelend \DIFaddbegin \DIFadd{p. }\DIFaddend 52). The \DIFdelbegin \DIFdel{containment }\DIFdelend \DIFaddbegin \DIFadd{inclusion }\DIFaddend $ \operatorname{Box}^w(c\epsilon) \subset B(\epsilon,p)$ shows that $ B(\epsilon, p) $ is a neighborhood of $ p $, which means that the \DIFdelbegin \DIFdel{accesible }\DIFdelend \DIFaddbegin \DIFadd{accessible }\DIFaddend points from $ p $ through horizontal curves form a neighborhood of $ p. $ Let $ q\in M $ be another arbitrary point on the connected manifold, and take a smooth curve $ \gamma $ connecting $ p $ and $ q. $  

	The image of $ \gamma $ is compact, so we can cover it with finitely many \DIFdelbegin \DIFdel{succesive }\DIFdelend \DIFaddbegin \DIFadd{successive }\DIFaddend box neighborhoods, where the Ball-Box theorem holds. Denote these neighborhoods as $ U_1,\dots,U_n, $ with each one of these boxes centered on \DIFdelbegin \DIFdel{succesive }\DIFdelend \DIFaddbegin \DIFadd{successive }\DIFaddend points $ p_i $ along the image of $ \gamma, $ and $ p_1=p, $ $ p_n=q. $ Moreover, these open sets can be chosen in \DIFdelbegin \DIFdel{orden }\DIFdelend \DIFaddbegin \DIFadd{order }\DIFaddend to satisfy $ U_i\cap U_{i+1}\neq\emptyset, $ so that there exists $ q_i \in U_i\cap U_{i+1}. $ By the Ball-Box theorem, we have paths connecting $ p_i $ with $ q_i, $ and $ q_i $ with $ p_{i+1}. $ Concatenating these paths yields a piecewise horizontal path connecting $ p $ and $ q. $ 
...
\DIFdelbegin %DIFDELCMD < \begin{df}[\cite{montgomery2002tour}, definition 11.1.1]
%DIFDELCMD < 	%%%
...
\end{rem}
\DIFdelbegin %DIFDELCMD < \begin{prop}[\cite{kobayashi1963foundations}, proposition 3.1]%%%
...
\DIFdelbegin %DIFDELCMD < \begin{df}[\cite{montgomery2002tour}, definition 11.2.1]
%DIFDELCMD < 	%%%
...
\DIFdelbegin %DIFDELCMD < \begin{df}[\cite{montgomery2002tour}, definition A.4.1]
%DIFDELCMD < 	%%%
...
\DIFdelbegin %DIFDELCMD < \begin{df}[\cite{arnol2013mathematical}, page 203]
%DIFDELCMD < 	%%%
...
Now, if $ M $ is a manifold with a sub-Riemannian structure $ \left( \mathcal{H}, \langle \cdot,\cdot \rangle \right)  $, we can define a \textit{cometric}, that is just a metric over the fibers of the cotangent bundle as follows. Given a metric $ \langle \cdot,\cdot \rangle_q $ for $ \mathcal{H}_q, $ for any $ v^*\in T^*_qM $ there exists a single $ V\in \mathcal{H}_q $ such that \DIFdelbegin \DIFdel{$ v^*|_{\mathcal{H}_q} = \langle V,\cdot \rangle_q $}\DIFdelend \DIFaddbegin \DIFadd{$ v^*|_{\mathcal{H}_q} =:\iota^*(v^*)= \langle V,\cdot \rangle_q $, where $ \iota: \mathcal{H}_q \hookrightarrow T_qQ $ is the inclusion, and $ \iota^*: T_q^*Q \rightarrow \mathcal{H}_q^* $ is its pullback}\DIFaddend . With this property, we can define the \DIFaddbegin \DIFadd{sub-Riemannian }\DIFaddend cometric $ (\cdot,\cdot)\in \mathfrak{X}_2^0(M) $ as a bilinear form (in general it is not a metric) over the fibers of the cotangent space as follows:
...
\DIFdelbegin %DIFDELCMD < \begin{df}[\cite{montgomery2002tour}, definition 1.5.2]
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \begin{df}[\cite{montgomery2002tour}, Definition 1.5.2]
	\DIFaddend If $ M $ is a manifold, and $ \left( \mathcal{H}, \langle\cdot,\cdot\rangle \right) $ is a sub-Riemannian structure over it, we define the sub-Riemannian Hamiltonian \DIFdelbegin \DIFdel{$ H_{sR}: T^*M \rightarrow \mathbb{R} $ }\DIFdelend \DIFaddbegin \DIFadd{$ H_{\rm sR}: T^*M \rightarrow \mathbb{R} $ }\DIFaddend as $$ H_{\DIFaddbegin \rm \DIFaddend sR}(p,q)= \frac{1}{2} (p,p)_q, $$   	
...
The sub-Riemannian Hamiltonian \DIFdelbegin \DIFdel{$ H_{sR}: T^*Q \rightarrow \mathbb{R}$ }\DIFdelend \DIFaddbegin \DIFadd{$ H_{\rm sR}: T^*Q \rightarrow \mathbb{R}$ }\DIFaddend can be written in terms of the Riemannian Hamiltonian $ H_R: T^*M \rightarrow \mathbb{R} $ in the following way: given the splitting $ TQ= \mathcal{H}\oplus V, $ we have that $ T^*Q = \mathcal{H}^* \oplus V^*, $ and so we have the canonical projection $ \operatorname{pr}_1: T^*Q \rightarrow \mathcal{H}^*. $ Now, we define a second map $ \operatorname{pr}_2: \mathcal{H}^* \rightarrow T^*M $ by the formula $ \operatorname{pr}_2(\alpha)(v)=\alpha(h_q v), $ where $ \alpha\in \mathcal{H}^*_q, $ $ v\in T_{\pi(q)}M, $ and $h_qv$ is the horizontal lift of $ v $ into $ \mathcal{H}_q $ (well-defined and unique, because $ \mathcal{H} $ is an Ehresmann connection).
\DIFdelbegin %DIFDELCMD < \begin{prop}[\cite{montgomery2002tour}, proposition 11.1.5]
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \begin{prop}[\cite{montgomery2002tour}, Proposition 11.1.5]
	\DIFaddend The induced sub-Riemannian Hamiltonian is given by \DIFdelbegin \DIFdel{$H_{sR}= H_R \circ \operatorname{pr},$ }\DIFdelend \DIFaddbegin \DIFadd{$H_{\rm sR}= H_R \circ \operatorname{pr},$ }\DIFaddend where $ \operatorname{pr}= \operatorname{pr_2}\circ \operatorname{pr_1}. $ 
\end{prop}
...
\DIFdelbegin %DIFDELCMD < \begin{df}[\cite{montgomery2002tour}, definition 11.2.3]
%DIFDELCMD < 	%%%
...
\begin{prop}\label{prop:essential_fact1}
	If $ g $ is a bi-invariant metric on a Lie group $ G, $ its geodesics \DIFaddbegin \DIFadd{that pass }\DIFaddend through the identity \DIFaddbegin \DIFadd{element }\DIFaddend coincide with its one-parameter subgroups.
\end{prop}
\begin{proof}
...
\begin{df}[\cite{montgomery2002tour}, p. 212]\DIFaddbegin \label{df:connection_one_form}
	\DIFaddend Let $ \pi:Q \rightarrow M $ be a principal $ G $-bundle with connection $ \mathcal{H} $. \textit{The connection one-form on $ Q $}, denoted as $ A, $ is a Lie-algebra-valued one-form on $ Q $, that is, $ A_q: T_qQ \rightarrow \mathfrak{g} $,  which satisfies the following conditions:
...
\end{tikzcd}
\end{center}
\DIFdelbegin \DIFdel{of vector bundles over $ M. $ }\DIFdelend This sequence is called the \textit{Atiyah sequence.}  
...
\DIFdelbegin %DIFDELCMD < \begin{df}[\cite{montgomery2002tour}, definition 4.1.1]
%DIFDELCMD < 	%%%
...
\DIFdelbegin %DIFDELCMD < \begin{cor}[First Wong's equation]%%%
...
\begin{lm}[\cite{montgomery2002tour}, Lemma 12.3.1]
	...
	\end{align}
	where $\left[ \xi, \chi \right]$ 	represents the corresponding point-wise bracket of the associated $ G $-equivariant maps of $ Q \rightarrow \mathfrak{g} $ of $ \xi,\chi, $ \DIFdelbegin \DIFdel{while }\DIFdelend \DIFaddbegin \DIFadd{$\Omega$ is the curvature form (Definition \ref{df:curvature_form}), and }\DIFaddend $ \tilde f : M \rightarrow \mathbb{R} $ is the function that makes the diagram of Definition \ref{df:lifted_configuration_function} commutative.
\end{lm}
\begin{proof}
...
\end{align*}
If $ f= \varphi\circ\pi_{T^*Q}=:q $ becomes the coordinate chart for $ Q $, we get that \DIFaddbegin \DIFadd{in coordinates 
}\DIFaddend $$ \dot{q} = g^{ij}P_i \DIFdelbegin %DIFDELCMD < \underbrace{hX_j(q)}%%%
\DIFdel{_{\text{in coordinates}}}\DIFdelend \DIFaddbegin \DIFadd{hX_j(q)}\DIFaddend , $$ 
...
by equation \eqref{eq:horizontal-horizontal}\DIFaddbegin \DIFadd{, where $ \Omega $ is the curvature form}\DIFaddend . From this discussion we derive the second Wong \DIFdelbegin \DIFdel{'s }\DIFdelend equation.

\DIFdelbegin %DIFDELCMD < \begin{cor}[Second Wong's equation]%%%
\DIFdelend \DIFaddbegin \begin{cor}[Second Wong equation]\DIFaddend \label{cor:swe}
	...
	$$ \nabla_{\dot{c}}\dot{c} = -( \lambda(i_{\dot{c}}\Omega))^{\DIFdelbegin \DIFdel{\#}\DIFdelend \DIFaddbegin \DIFadd{\sharp}\DIFaddend }, $$ 
	where $ c = \pi\circ\pi_{T^*Q}\circ\alpha, $ $\lambda = [\pi_{T^*Q}(\cdot), \eta^*_{\pi_{T^*Q}(\cdot)}(\cdot)], $ and \DIFdelbegin \DIFdel{$ \# $ }\DIFdelend \DIFaddbegin \DIFadd{$ \sharp $ }\DIFaddend represents lowering indices using the metric on $ M. $ In coordinates this equation is
...
If the structure group $ G $ of the principal $ G $-bundle $ \pi: Q \rightarrow M $ is abelian, then the adjoint and co-adjoint bundles are the trivial bundles \DIFdelbegin \DIFdel{$ M\times \mathfrak{g}^{(*)} \rightarrow M, $ given that $ \operatorname{ad}^{(*)} : G \rightarrow \operatorname{Aut} ( \mathfrak{g}^{(*)}) $ }\DIFdelend \DIFaddbegin \DIFadd{$ M\times \mathfrak{g} \rightarrow M $ and $M\times \mathfrak{g}^* \rightarrow M$, respectively. This is because $ \operatorname{ad} : G \rightarrow \operatorname{Aut} ( \mathfrak{g}) $ }\DIFaddend satisfies \DIFdelbegin \DIFdel{$ \operatorname{ad^{(*)}} (g)= 1_{ \mathfrak{g}^{(*)}} $ }\DIFdelend \DIFaddbegin \DIFadd{$ \operatorname{ad} (g)= 1_{ \mathfrak{g}} $ }\DIFaddend for all \DIFdelbegin \DIFdel{$ g\in G. $ }\DIFdelend \DIFaddbegin \DIFadd{$ g\in G $. }\DIFaddend The \DIFaddbegin \DIFadd{reasoning is identical for the co-adjoint bundle. The }\DIFaddend sections of these bundles correspond to functions \DIFdelbegin \DIFdel{$ M \rightarrow \mathfrak{g}^{(*)} $}\DIFdelend \DIFaddbegin \DIFadd{from $M$ to the (dual}\DIFaddend , \DIFaddbegin \DIFadd{for the co-adjoint bundle) Lie algebra, }\DIFaddend because sections are maps of the form $ 1_M\times f, $ where \DIFdelbegin \DIFdel{$ f: M \rightarrow \mathfrak{g}^{(*)}.$ 
}\DIFdelend \DIFaddbegin \DIFadd{$ f: M \rightarrow \mathfrak{g}, \mathfrak{g}^*.$  
}\DIFaddend 
