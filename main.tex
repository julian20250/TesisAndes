...
\begin{df}[\cite{montgomery2002tour}, definition A.4.1]
	Given a symplectic manifold  $ (M,\omega) $, a \textit{Hamiltonian} is a smooth function $ H: M \rightarrow \mathbb{R}. $
\end{df}
...
Any manifold $ M $ has a symplectic manifold associated: the cotangent bundle. Suppose that $ M $ is an $ n$-dimensional manifold. $ T^*M= \bigsqcup_{p\in M} T^*_p M$ is then a $ 2n $-dimensional manifold. There is a canonical choice of coordinates for this space, namely, given $ x\in M, $ there is a coordinate chart $ (U_x,\phi) $ on $ M $ such that $ q=\phi(x)=(q_1,\dots,q_n)\in \mathbb{R}^n, $ and for a given 1-form $ \alpha\in T_x^*M,$ in these coordinates we have that $ \alpha=p_i dq^i; $ mixing these two coordinates we get the \textit{canonical coordinates of $ T^*M $}, where a given element $ \alpha\in T^*M $ can be represented locally as the pair of coordinates $ (q,p). $      
...
\begin{proof}
	Let $ v\in T_p \left( T^*M \right)  $ be a vector tangent to the cotangent bundle at $ \alpha\in T^*_x M. $ If $ \pi:T^*M \rightarrow M $ is the natural projection, the differential $ d\pi : T \left( T^*M \right) \rightarrow TM $ takes $ v $ to $ d\pi(v)=(\pi(\alpha), d\pi_\alpha(v))=(x,d\pi_\alpha(v)), $ which is a vector tangent to $ M $ at $ x. $ We define a 1-form $ \theta $ on $ T^*M $ by the relation $ \theta(v)=\alpha(d\pi_\alpha(v)). $ In local coordinates this form is $ \theta= p_i dq^i, $ and the form $ \omega = d\theta $ is non-degenerate. 
\end{proof}
...
\end{equation}
This system of differential equations receives the name of \textit{Hamilton equations for $ H $.} Despite this formulation on the cotangent bundle came after Hamilton, these equations are known by its name because they are a generalization of the ones that are obtained using the generalized momentums instead of the classical velocity and position variables in classical mechanics. Moreover, this method turns out to be a generalization of Hamiltonian mechanics to symplectic manifolds.
...
\end{prop}
\begin{proof}
	(\cite{poisson}, proof of proposition 1.3) Properties 1,2 and 4 are trivial. The third property, known as the \textit{Jacobi identity},  is equivalent to the fact that $ \omega $ is a closed form ($ d\omega =0 $), as will be shown next. Recall that $$ d\chi(X,Y,Z)=X\chi(Y,Z)-Y\chi(X,Z)+Z\chi(X,Y)-\chi([X,Y],Z)+\chi([X,Z],Y)-\chi([Y,Z],X) $$ 
...
\begin{thm}[Theorem on normal geodesics of bundle type subriemannian metrics, \cite{montgomery2002tour}, theorem 11.2.5]
	%kk
	Let $ Q $ be the total space of a principal $G $-bundle $ \pi: Q \rightarrow M $, endowed with a subriemannian metric of bundle type. Consider an extension of this metric to a compatible Riemannian or pseudo-Riemannian metric whose associated vertical metric has constant bi-invariant type. Take any geodesic for the extended metric and project it to the base space $ M. $ Horizontally lift the result back to $ Q. $ The result is a normal subriemannian geodesic. All normal subriemannian geodesics can be obtained in this manner. The projected curve is a geodesic in the base $ M $ if and only if the original curve is a horizontal geodesic in the total space $ Q. $ 
\end{thm}
...
\begin{proof}
	...
	\end{enumerate}
	\begin{proof}
		[Proof of the claims] It suffices to show that the claims hold over any locally trivialized neighborhood $ \pi^{-1}(U) $ in $ Q. $ Let $ \phi_U: \pi^{-1}(U) \rightarrow U\times G $ be the corresponding local trivialization, which induces a diffeomorphism $ T^*Q \cong T^*U\times T^*G,$ that takes the Poisson bracket to the product Poisson bracket. 
...
\end{tikzcd}
	...
	Now, to show that the projected curve is a geodesic in the base $ M $ if and only if the original curve is a horizontal geodesic in the total space $ Q $, we use the fact that the distance between points in $ M $ is the distance between fibers in $ Q, $ so that $ \pi(\gamma(t) ) $ is a geodesic if and only if $\gamma(t)$ minimizes distances between fibers, but this implies that this curve is normal to the fibers (due to the first variation of arc length), and therefore it is a subriemannian geodesic.  
\end{proof}
% ===============================================================
