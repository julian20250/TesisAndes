...
\end{df}
The $ G $-invariant vector fields on $ Q $ can be identified with sections of $ (TQ)/G, $ and due to Proposition \ref{prop:splitting} we can split them up into horizontal and vertical vector fields, according to $ (TQ)/G = TM \oplus \operatorname{Ad} (Q). $ Then, if $ X\in \mathfrak{X}(M) $ and $ \xi: M \rightarrow \rho(Q), $ we have the following relations:
\begin{align}
