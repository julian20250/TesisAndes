...
%DIF ADD main.tex       Mon Jul 11 21:45:42 2022
% Title of Document: 
% Start Date: 
%%%%%%%%%%%%%%%%%%%%%%%%
%%%FINAL CHECK%%%%%%%%%
...
\begin{titlepage}
	...
	\textbf{ELEMENTS OF SUB-RIEMANNIAN GEOMETRY AND ITS APPLICATIONS}
        ...
        Bogotá, Colombia\\
        \DIFdelbegin \DIFdel{April 26}\DIFdelend \DIFaddbegin \DIFadd{July 11}\DIFaddend , 2022
...
\newpage
\begin{abstract}
	The goal of this work is to introduce the main theorems of sub-Riemannian geometry\DIFdelbegin \DIFdel{, and for that we will cover first the topic }\DIFdelend \DIFaddbegin \DIFadd{. We first cover the concept }\DIFaddend of distributions, stating the Frobenius \DIFdelbegin \DIFdel{' }\DIFdelend theorem and introducing the idea of foliation. Later on, we focus on \DIFdelbegin \DIFdel{the }\DIFdelend sub-Riemannian structures, and prove Chow \DIFdelbegin \DIFdel{'s }\DIFdelend theorem, central in the study of connectivity through horizontal geodesics. The last part of this \DIFdelbegin \DIFdel{work }\DIFdelend \DIFaddbegin \DIFadd{thesis }\DIFaddend focuses on $ G $-principal bundles endowed with compatible sub-Riemannian structures. We use this construction with ideas of symplectic geometry to prove the Theorem on normal geodesics of bundle type sub-Riemannian metrics and to study Wong \DIFdelbegin \DIFdel{'s }\DIFdelend equations. This review of sub-Riemannian topics is based mostly on \cite{montgomery2002tour}.
\end{abstract}
\newpage
...
Riemannian geometry is an intense field of study due to the fact that it is the generalization of the idea of metric and distance \DIFdelbegin \DIFdel{onto general geometric spaces }\DIFdelend \DIFaddbegin \DIFadd{from vector spaces onto manifolds}\DIFaddend . This characteristic makes Riemannian geometry one of the preferred formalisms to work in physics \DIFdelbegin \DIFdel{' }\DIFdelend topics like general relativity and quantum mechanics. In this field we start with a manifold, endowed with a metric, which is a section of the $ (0,2) $-tensor bundle \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend is symmetric and non-degenerate at all points of the manifold. The non-degeneracy condition can be weakened, and this variation of the definition of Riemannian metric arises the field of study known as pseudo-Riemannian metric. \DIFdelbegin \DIFdel{In this case some }\DIFdelend \DIFaddbegin \DIFadd{Some }\DIFaddend important examples raise up \DIFaddbegin \DIFadd{in this alternative theory}\DIFaddend , like the Lorentz/Minkowski geometry, the most widely used metric in special relativity. An extensive mathematical development of pseudo-Riemannian geometry can be found on \cite{o1983semi}.

Another variation of Riemannian metric arises when the condition of being a section of the $(0,2)$-tensor bundle is weakened to consider instead a continuous map $F: TM \rightarrow \mathbb{R}$, smooth on the set of nonzero vectors, whose restriction to each tangent space $T_pM$ is a norm \cite[p.~47]{lee2018introduction}. This definition is known as Finsler metric\DIFdelbegin \DIFdel{, and an }\DIFdelend \DIFaddbegin \DIFadd{. An }\DIFaddend approach to the consequences of this definition can be found on \cite{bao2012introduction}.

The variation of  \DIFdelbegin \DIFdel{Riemannian geometry }\DIFdelend \DIFaddbegin \DIFadd{the Riemannian geometry theory }\DIFaddend that will be covered throughout this text arises when we consider a sub-bundle (called distribution) of the tangent bundle of a given manifold: in this case, the metric will be a section of the $ (0,2) $-tensor bundle of this sub-bundle, with the same properties \DIFdelbegin \DIFdel{than in the case of }\DIFdelend \DIFaddbegin \DIFadd{as the ones used in }\DIFaddend Riemannian geometry (non-degeneracy and symmetry). This formulation receives the name of sub-Riemannian geometry, and it will be the main focus of this text. Before introducing the concept of sub-Riemannian structure, \DIFdelbegin \DIFdel{we focus }\DIFdelend in Section \ref{sec:1} \DIFaddbegin \DIFadd{we focus }\DIFaddend on explaining the idea of distribution, their different definitions and associated results. To do this, we define a distribution in terms of local frames and 1-forms, we explain when a distribution is involutive, integrable and completely integrable, and use the Frobenius \DIFdelbegin \DIFdel{'s }\DIFdelend theorem to connect these three concepts, while discussing when the points of a given manifold are connected via horizontal curves, \textit{i.e.}, curves whose tangent vector is always contained in the distribution.

Later on, in Section \ref{sec:sub-Riemannian_geometry} we introduce the concept of sub-Riemannian structure, and define the horizontal distance as the infimum of the length of all horizontal curves that connect two given points in the manifold, in analogy with Riemannian distance. We then introduce the bracket-generating distributions as a special class of distribution such that there is a local frame that locally spans the tangent bundle through successive applications of Lie brackets, and we state and show the Chow \DIFdelbegin \DIFdel{'s }\DIFdelend theorem, which states that a sufficient condition for all points to be connected through horizontal curves is that the distribution is bracket-generating. Therefore, the distance between two points in a sub-Riemannian structure is always well-defined when the subjacent distribution is bracket-generating. 

In order to work with principal $ G $-bundles endowed with metric of bundle type, \DIFdelbegin \DIFdel{in Section \ref{sub:metrics_on_principal_bundles} we start }\DIFdelend \DIFaddbegin \DIFadd{Section \ref{sub:metrics_on_principal_bundles} starts }\DIFaddend introducing what is a submersion, and \DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{explains how to }\DIFaddend endow it with a sub-Riemannian structure in the total space (equivalently, a Riemannian structure in the base space). With these tools, we start the topic of metrics on principal bundles explaining aspects of Lie groups and their actions, and we give some examples of principal $ G $-bundles in Section \ref{sub:examples_of_principal_g_bundles}. 
...
\begin{df} 
	If $ (E,M,\pi:E \rightarrow {M}) $ is a smooth vector bundle, the \DIFdelbegin \DIFdel{dual bundle }\DIFdelend \DIFaddbegin \textit{\DIFadd{dual bundle}} \DIFaddend of this bundle is another smooth vector bundle $ (E^*,M,\pi^*:E^*\rightarrow {M}) $, \DIFdelbegin \DIFdel{known as }\textit{\DIFdel{dual bundle}}%DIFAUXCMD
...
The concept of distribution can be defined in an alternative but equivalent  \DIFaddbegin \DIFadd{fashion using 1-forms }\DIFaddend (see \cite{lee2003introduction}, Ch. 19, lemma 19.5) \DIFdelbegin \DIFdel{fashion using 1-forms, }\DIFdelend with the help of the following proposition.
...
The curves that are considered in \DIFdelbegin \DIFdel{Definition }\DIFdelend \DIFaddbegin \DIFadd{definition }\DIFaddend \ref{df:horizontal_distance} are required to be absolutely continuous, which is a weaker condition than smoothness in the sense that the \DIFaddbegin \DIFadd{former kind of }\DIFaddend curve is differentiable in almost all points of its domain, not necessarily in all its domain. Nevertheless, the distance remains the same if we consider instead the set of all smooth curves (see \cite{montgomery2002tour}, page 23), but the motivation to consider the bigger set of absolutely continuous curves is due to the fact that the curves that realize the distance between two points are not always smooth.
...
\subsection{Chow \DIFdelbegin \DIFdel{'s }\DIFdelend Theorem}%
...
With the Ball-Box theorem, Chow \DIFdelbegin \DIFdel{'s }\DIFdelend theorem can be derived easily.
...
\begin{prop}\label{prop:essential_fact1}
	If $ g $ is a bi-invariant metric on a Lie group $ G, $ its geodesics \DIFaddbegin \DIFadd{that passes }\DIFaddend through the identity \DIFaddbegin \DIFadd{element }\DIFaddend coincide with its one-parameter subgroups.
\end{prop}
\begin{proof}
...
\DIFdelend \DIFaddbegin \textit{\DIFadd{non-abelian Lorentz equations,}} \DIFaddend also known as \DIFdelbegin \textit{\DIFdel{Wong's equations}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \textit{\DIFadd{Wong equations}} \DIFaddend (\cite{montgomery2002tour}, p. 209). Moreover, the results of this section are useful to find the sub-Riemannian geodesics in cases where Theorem \ref{thm:normal_geodesics} can not be applied, which may happen when the Riemannian geodesics on $ M $ are not known, or are harder to compute than the sub-Riemannian geodesics.
...
\begin{proof}
	...
	\end{itemize}
\end{proof}
\DIFdelbegin \subsection{\DIFdel{Wong's equations}}%DIFAUXCMD
...
\end{prop}
The isomorphism between $ TQ/G $ and $ TM \oplus \operatorname{Ad} (Q) $ given in \DIFdelbegin \DIFdel{Proposition }\DIFdelend \DIFaddbegin \DIFadd{proposition }\DIFaddend \ref{prop:splitting} can be dualized to obtain the isomorphism $ T^*Q/G \rightarrow T^*M \oplus \operatorname{Ad}^*(Q) $ given by
$$ \left[ \alpha \right] \mapsto h^*_q\alpha \oplus \left[ q, \eta_q^*(\alpha)\right], \ q=\pi_{T^*Q}(\alpha). $$ 
By \DIFdelbegin \DIFdel{Proposition }\DIFdelend \DIFaddbegin \DIFadd{proposition }\DIFaddend \ref{prop:momentum_map}, if $ \alpha: I \rightarrow T^*Q $ is a sub-Riemannian geodesic, $ J(\alpha(t)) $ is constant. We denote as $ \lambda: I \rightarrow \operatorname{Ad}^*(Q) $ the curve that makes the following diagram commutative,
\begin{center}
