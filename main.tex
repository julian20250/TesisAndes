...
\begin{df}[\cite{montgomery2002tour}, p. 212]
	Let $ \pi:Q \rightarrow M $ be a principal $ G $-bundle with connection $ \mathcal{H}, $ that is, $ (d\alpha_g)_q( \mathcal{H}_q) = \mathcal{H}_{q\cdot g}. $ \textit{The connection one-form on $ Q $}, denoted as $ A, $ is a Lie-algebra-valued one-form on $ Q $, that is, $ A_q: T_qQ \rightarrow \mathfrak{g} $,  which satisfies the following conditions:
	...
	\end{enumerate}
\end{df}
The second condition of previous definition is a normalization condition: as the image of $ \eta_q $ is $ V_q, $ when restricting to the vertical $ A_q $ is the canonical identification between $ V_q $ and $ \mathfrak{g} $. Both conditions guarantee that the connection one-form is unique, and that it satisfies the following equivariance condition:
