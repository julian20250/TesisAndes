...
\begin{df}
	...
	$ a submersion, we say that the pair $ (\pi, g_Q) $ is a \textit{riemannian submersion} if the isomorphism $ d\pi_q |_{\mathcal{H}_q} $ is an isometry between $ T_m M $ and $ \mathcal{H}_q ,$ with $ q\in Q_m $. 
\end{df}

The previous definition frees us from the problem of the dependence of the inner product induced on $ T_mM $ by the  vertical space $ V_q $ of the point $ q\in Q_m $ chosen, because the isometry between $ T_mM $ and $ \mathcal{H}_q $ guarantees that the inner product is independent of the element $ q. $
...
If we define the action $ \alpha: (M\times G) \times G \rightarrow M\times G $ as $ ((p, g),q)\mapsto (p,gq)  $, it is clear that $ G $ acts freely on $ M\times G $, $\iota(p, gh) = \iota(p,g)\cdot h $ (the local trivialization is equivariant with respect to the action) and $ \pi^{-1}(p) = \left\{ p \right\}\times G = G_{(p,g)} $, so that $ \pi $ is a principal $ G $-bundle. A connection for this bundle is the distribution $ \mathcal{H}_{(p,g)}\subset T(M\times G) $ transverse to the vertical $ V_{(m,g)} \cong T_g G $, that is, the connection is tangent to the foliation $ M\times \left\{ g \right\} $ at every point $ (m,g)\in M\times G. $
