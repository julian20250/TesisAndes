...
\begin{df}
	...
	\begin{itemize}
		\item $ G $ acts freely and properly on $ Q $.
...
\begin{df}[\cite{montgomery2002tour}, page 196]
	...
	$ with Ehresmann connection $ \mathcal{H} $ is called a \textbf{principal $ G- $bundle with connection} if it is a principal $ G- $bundle such that the $ G- $action on $ Q $ preserves the horizontal distribution $ \mathcal{H} $ in the sense that $ \mathcal{H}_{q\cdot g} = d(\alpha_g)_q (\mathcal{H}_q), $ for all $ q\in Q, $ $ g\in G $.
\end{df}
...
\begin{ex}[Hopf Fibration]
	Recall that all rotations in $ \mathbb{R}^3 $ can be expressed in a simple way with help of quaternions. Every rotation around an axis given by a vector $ \vec{v}=(b,c,d) $ and an angle $ \theta $ is identified with an unitary quaternion $r = a+bi+cj+dk, $ where $ \theta = 2\arccos(a) $ (see \cite{lyons2003elementary}, page 6). This rotation $ R_r: \mathbb{R}^3 \rightarrow\mathbb{R}^3 $ is given by $ x\mapsto rxr^{-1} $, with $ r^{-1} = r^*/|r|, $ and $ x $ identified as a pure quaternion, that is, $ x=x_1i+x_2j+x_3k. $   The set of all unitary quaternions can be identified with $ \mathbb{S}^3 $, so that $ \mathbb{S}^3 $ acts on $ \mathbb{R}^3 $ via rotations. The Hopf projection is defined as the map $ \pi: \mathbb{S}^3 \rightarrow \mathbb{S}^2 $ that sends the point $ (1,0,0) $ to its respective image after rotating $ \mathbb{R}^3 $ with the associated quaternion, as an element of $ \mathbb{S}^3 $. That is, $ \pi(a,b,c,d) = R_r(1,0,0), $ where $ r=a+bi+cj+dk. $  

	It is clear that the function $ \pi $ defined above is surjective, as every point $ x\in\mathbb{S}^2 $ can be reached from $ (1,0,0) $ through an adequate rotation. Explicitly, it suffices to take a normal vector to both $ (1,0,0), x $ as the axis of rotation, and the angle of rotation is the angle between both vectors. 

	If we identify $ \mathbb{R}^4=\mathbb{C}\times \mathbb{C} $ and $ \mathbb{R}^3= \mathbb{R}\times\mathbb{C}  $, Hopf projection from $ \mathbb{S}^3= \{ (z_1,z_2)\  |$ $ \ z_1,z_2\in \mathbb{C}, \ |z_1|^2+|z_2|^2=1 \} $ to $ \mathbb{S}^2 = \{ (x,z)\ | \ z\in \mathbb{C},\ x\in \mathbb{R},\ |z|^2+$ $x^2=1 \} $  can be written as $ \pi: \mathbb{S}^3 \rightarrow { \mathbb{S}^2},$ $ \pi(z_1,z_2) =(|z_1|^2-|z_2|^2, 2z_1z_2^* ).$ In this form one can show that this map is a submersion analyzing the minors of the jacobian matrix in cartesian coordinates (see \cite{linintroductory}, proof of theorem 3.11).

	To identify the fibers of this submersion, notice that $ \pi^{-1}(1,0) = \left\{ (w_1,0) \ | \ |w_1| =1 \right\} = \left\{ (e^{i\theta},0) \ | \ \theta\in (0,2\pi]  \right\} $. Now, in quaternion notation, if $ s,r\in \pi^{-1}(x), $ $ rir^{-1} = sis^{-1}=x $, that is, $ (s^{-1}r)i(r^{-1}s) = i $. Then, $ s^{-1}r \in \pi^{-1}(1,0), $ and $ r= se^{i\theta}, $ so $ \pi^{-1}(x) = \left\{ re^{i\theta}\ | \ \theta\in (0,2\pi] \right\} $, and each fiber is diffeomorphic to $ \mathbb{S}^1. $        

	With this, $ \mathbb{S}^3 $ can be seen as the disjoint union of these circles, and endowing this disjoint union with the usual manifold structure of $ \mathbb{S}^3 $, we can take $ U= \mathbb{S}^2\setminus \left\{ m \right\}, $ with $ m\in \mathbb{S}^2 $ arbitrary, and define a diffeomorphism $ \phi: \pi^{-1}(U) \rightarrow {U\times \mathbb{S}^1}
	 $ such that $ \pi: \mathbb{S}^3 \rightarrow { \mathbb{S}^2}
	  $ is a fiber bundle with fiber $ \mathbb{S}^1. $ 

	Now, we define an action $ \alpha: \mathbb{S}^3\times \mathbb{S}^1 \rightarrow { \mathbb{S}^3}
	$, given by $ \alpha(z_1,z_2,\lambda) = (z_1\lambda, z_2\lambda), $ where $ (z_1,z_2)\in \mathbb{S}^3\subset \mathbb{C}^2 $ and $ \lambda\in \mathbb{S}^1 \subset \mathbb{C}. $ This action acts freely and properly on $ \mathbb{S}^3, $ and its orbits coincide with the fibers of $ \pi, $ so that $ \mathbb{S}^3/ \mathbb{S}^1 \cong \mathbb{S}^2. $ Therefore, $ \pi: \mathbb{S}^3 \rightarrow { \mathbb{S}^2}
	 $ is a principal $ \mathbb{S}^1- $bundle.

	 To find an Ehresmann connection for this submersion, we recall that, by the preimage of regular value theorem, $ \pi^{-1}(m) = \mathbb{S}^1_q $ is a submanifold of $ \mathbb{S}^3 $, of dimension $ \operatorname{dim}( \mathbb{S}^3)- \operatorname{dim}( \mathbb{S}^2) = 1,  $ and its tangent space (vertical space of the submersion) $ T_p \mathbb{S}^1_q $ is $ \operatorname{Ker}(d\pi_p),  $ and any complementary space (with respect to $ T \mathbb{S}^3 $)  to this tangent space would be an Ehresmann connection. Now, if $ \mathbb{S}^3 $ is endowed with the usual riemannian structure, there is only one Ehresmann connection compatible with the metric, and is given by $ \mathcal{H}_q = \operatorname{Ker}(d\pi_p)^\perp. $ 

	 This principal $ \mathbb{S}^1- $bundle has connection because the kernel of $ d(\pi\circ \alpha_g)_q $ is the same of $ d\pi_q, $ as $ d\alpha_g $ is an isomorphism. Therefore, $ d(\alpha_g)_q ( \mathcal{H}_q) = \mathcal{H}_{q\cdot g}, $ and the $ \mathbb{S}^1- $action preserves $ \mathcal{H}. $ 
