...
\begin{df}[\cite{arnol2013mathematical}, section 8A]
	...
	$$ d\omega=0 \text{ and } \forall \xi \neq 0\ \exists \eta: \ \omega_x(\xi,\eta)\neq0 \ (\xi,\eta\in T_x M).$$ 
...
$$ \dot{q^i} = \frac{\partial H}{\partial p_i}, \quad \dot{p_i} =- \frac{\partial H}{\partial p_i}.   $$ 
...
\begin{df}[\cite{montgomery2002tour}, example 2, section A.4]
	For any vector field $ \xi\in \mathfrak{X}(M), $ we define its momentum function $ P_\xi:T^*M \rightarrow \mathbb{R} $ by
	$$ P_\xi(q,p)= p(\xi_q). $$ 
\end{df}

\begin{ex}[\cite{montgomery2002tour}, example 1, section A.4]
	If $ \xi=\partial q^i\in \mathfrak{X}(M), $ its momentum function will be	
	$$ P_{\partial q^i}(q,p)=p(\partial q_i) = p_i. $$ 
	In the other hand, if we take the Hamiltonian $ H(p,q)=p_i, $ we get that $ X_H=\partial q^i. $ This says that the \textit{i}-th momentum generates translation in the \textit{i}-th position variable. 
\end{ex}
