...
\begin{ex}[Heisenberg Group]\label{ex:heis^group}
...
\begin{proof}
	(Taken from \cite{lee2003introduction}) First, it will be shown that any involutive distribution is spanned by independent smooth commuting\footnote{It is said that two vector fields commute if its Lie bracket is zero. A set of smooth vector fields is commuting if the Lie bracket of every pair is zero.} vector fields, and as a consequence of this fact, the distribution is completely integrable.
	...
	Let $ \pi: \mathbb{R}^m \rightarrow  \mathbb{R}^k$ the projection onto the first $ k $ coordinates, that is, $ \pi(x^1,\dots,x^m)=(x^1,\dots,x^k). $ This map induces a smooth\footnote{Smooth because it is the composition $\mathcal{H}' \hookrightarrow T\varphi(U) \xrightarrow[]{d\pi} T\pi(\varphi(U))
	$ }  bundle homomorphism $ d\pi: T \mathbb{R}^m \rightarrow \mathbb{R}^k	 $ that acts as follows:
