...
A manifold without boundary $M$ of dimension $ m $  is said to be of class $ C^\infty $ if the differentiability class of the transition maps is $C^\infty.$ In this case, for every $ p\in M $ one can define the tangent space to \todo{to $M$ at $p$} $ p $, $ T_p M $, as the set of all derivations of the algebra of germs of smooth functions from $ M $ to $ \mathbb{R}  $,  based on $ p $ \todo{based at $p$}. This definition of tangent space is equivalent (when the differentiability class of $ M $ is $ \infty $) to the equivalence class of germs of curves that passes through $ p $. If a manifold $ M $  is $C^r$, with $r< \infty$, the definition for tangent space is the latter, so we use that definition for the tangent space throughout this text, independently of the differentiability class.

Joining\todo[inline]{We take the disjoint union of} the tangent spaces of every point of the manifold induces\todo{“induces”?} the tangent bundle,
$$ TM = \bigsqcup_{p\in M} T_pM, $$ 
equipped with manifold structure given by the one of $ M. $ It is called a bundle because the triple $ (TM, M,\pi:TM \rightarrow {M}) $ is in fact a bundle\todo[inline]{Ok, but what is “a bundle”?}, where $ \pi:TM \rightarrow {M} $  is the projection function, that assigns each tangent vector to the point where it is based. Each fiber of this bundle is a tangent space based on a point, and the tangent bundle is in particular a vector bundle because each fiber has structure of finite dimensional real vector space. Moreover, the tangent bundle belongs to the class of smooth vector bundles, objects of particular interest in differential geometry.
...
For an arbitrary bundle $ (E,B,\pi:E \rightarrow {B}), $ a subbundle of this bundle $ (E',B',\pi':E' \rightarrow {B'}) $  is just \todo[inline]{what is it: “a bundle”?}a bundle that satisfies $ E'\subset E $, $ B'\subset B $ and $ \pi|_{E'}=\pi'. $ In particular, a smooth vector subbundle is a vector subbundle that is also smooth. 
...
\end{rem}
\begin{ex}[Vector Field over a Manifold]\todo[inline]{Why do you use capital letters?}\label{ex:vect_field}
Any smooth vector field $ X: M \rightarrow {TM} $ (which does not cancel out at any point)\todo[inline]{“cancel out” $\to$ “vanish”} on a manifold determines a distribution $ \mathcal{H}$ over the manifold $ M $ whose fiber at an arbitrary $ p\in M $ is the span of $ X_p. $

As a particular case of this example, one can consider a system of $ n $ linear differential equations on $ \mathbb{R}^n $ without equilibrium points, that determines a vector field over $ \mathbb{R}^n,  $ and this in turn generates a distribution via the span of the vectors at each point\todo{????}.
\end{ex}
...
\begin{proof}(\cite{lee2003introduction}, Ch. 19)
...
The following are examples of foliations over a manifold, mainly taken from \cite{lee2003introduction}, Ch. 19.
...
Following the procedure of \cite{lee2003introduction}, Ch.19, the next lemma is going to be used in the proof of this theorem.
