...
\begin{ex}[Hopf Fibration]
	We identify $ \mathbb{R}^4=\mathbb{C}\times \mathbb{C} $ and $ \mathbb{R}^3= \mathbb{C}\times \mathbb{R}  $  to define the submersion from $ \mathbb{S}^3= \left\{ (z_1,z_2)\ | \ z_1,z_2\in \mathbb{C}, \ |z_1|^2+|z_2|^2=1 \right\} $ to $ \mathbb{S}^2 = \{ (z,x)\ | \ z\in \mathbb{C},\ x\in \mathbb{R},\ |z|^2+$ $x^2=1 \} $  as $ \pi: \mathbb{S}^3 \rightarrow { \mathbb{S}^2},$ $ \pi(z_1,z_2) =(2z_1z_2^*, |z_1|^2-|z_2|^2).$ One can show that this map is a submersion analyzing the minors of the Jacobian matrix in Cartesian coordinates, and it is also a surjective map, as for every $ (w,x)\in \mathbb{S}^2 $ we can take $ (z_1,z_2) = ( |w|\sqrt{1/(2x-2)}\cdot e^{i \operatorname{Arg}(w)/2 },  \sqrt{(1-x)/2}\cdot e^{-i \operatorname{Arg}(w)/2 }) $ to have $ \pi(z_1,z_2) = (w,x). $ 

	To identify the fibers of this submersion, let $ (z_1,z_2), (w_1,w_2)\in \pi^{-1}(w,x), $ so that $ (2z_1z_2^*, |z_1|^2-|z_2|^2)=(2w_1w_2^*, |w_1|^2-|w_2|^2). $ From $ |z_1|^2-|z_2|^2 = |w_1|^2-|w_2|^2 $ and $ |z_1|^2+|z_2|^2 = |w_1|^2+|w_2|^2=1$ we deduce that $ |z_i|=|w_i|, $ for $ i=1,2. $  From $ 2z_1z_2^*=2w_1w_2^*, $ we deduce that $ \operatorname{Arg}(z_1)- \operatorname{Arg}(z_2) =\operatorname{Arg}(w_1)- \operatorname{Arg}(w_2) $, and this is true only if $ \operatorname{Arg} (w_i) = \operatorname{Arg}(z_i)+\theta, $ for $ i=1,2, $ $ \theta\in \mathbb{R}. $ Moreover, as $ (e^{i\theta}z_1: e^{i\theta}z_2)\in \pi^{-1}(w,x), $ we have that

	\begin{align*}
		\pi^{-1}(w,x) = \{ (e^{i\theta}z_1, e^{i\theta}z_2) \ | \ (z_1,z_2)= ( |w|\sqrt{1/(2x-2)}\cdot e^{i \operatorname{Arg}(w)/2 },\\  \sqrt{(1-x)/2}\cdot e^{-i \operatorname{Arg}(w)/2 }), \theta\in [0,2\pi) \}.
	\end{align*}
	
	Therefore, $ \pi^{-1}(w,x)\cong \mathbb{S}^1, $ and $ \mathbb{S}^3 $ can be seen as the disjoint union of these circles. In fact, with this assertion we can take $ U= \mathbb{S}^2\setminus \left\{ m \right\}, $ with $ m\in \mathbb{S}^2 $ arbitrary, and define a diffeomorphism $ \phi: \pi^{-1}(U) \rightarrow {U\times \mathbb{S}^1}
